...
void main() {
    ...
    if (texture2D(texture_world, v_vTexcoord).w == 0.0) {
        ...
        vec4 phi_hat_next = texture2D(gm_BaseTexture, from) + (texture2D(texture_material_1, from) / 255.0);
        vec4 color;
        ...
        if (USE_MACCORMACK_SCHEME) {
            ...
            vec4 phi_hat_now = texture2D(gm_BaseTexture, to) + (texture2D(texture_material_1, to) / 255.0);
            
            color = phi_hat_next + ((texture2D(gm_BaseTexture, v_vTexcoord) + (texture2D(texture_material_1, v_vTexcoord) / 255.0)) - phi_hat_now) * maccormack_weight_half;
            
            vec2 coord = floor(from / texel_size + 0.5) * texel_size;
            vec4 top_left = texture2D(gm_BaseTexture, coord + precalculated_1.zw) + (texture2D(texture_material_1, coord + precalculated_1.zw) / 255.0);
            vec4 bottom_right = texture2D(gm_BaseTexture, coord + precalculated_1.xy) + (texture2D(texture_material_1, coord + precalculated_1.xy) / 255.0);
            vec4 top_right = texture2D(gm_BaseTexture, coord + precalculated_1.xw) + (texture2D(texture_material_1, coord + precalculated_1.xw) / 255.0);
            vec4 bottom_left = texture2D(gm_BaseTexture, coord + precalculated_1.zy) + (texture2D(texture_material_1, coord + precalculated_1.zy) / 255.0);
        ...
        gl_FragColor = mix(floor(color * 255.0) / 255.0, fract(color * 255.0), target);
    } else {
