...
void main(in PS_INPUT IN, out PS_OUTPUT OUT) {
    ...
    if (tex2D(texture_world, IN.uv).w == 0.0) {
        ...
        float phi_hat_next = unpack_ufloat_16(tex2D(gm_BaseTexture, from).wx);
        float color;
        ...
        if (USE_MACCORMACK_SCHEME) {
            ...
            float phi_hat_now = unpack_ufloat_16(tex2D(gm_BaseTexture, to).wx);
            
            color = phi_hat_next + (unpack_ufloat_16(tex2D(gm_BaseTexture, IN.uv).wx) - phi_hat_now) * precalculated_2.z;
            
            float2 coord = floor(from / texel_size + 0.5) * texel_size;
            float top_left = unpack_ufloat_16(tex2D(gm_BaseTexture, coord + precalculated_1.zw).wx);
            float bottom_right = unpack_ufloat_16(tex2D(gm_BaseTexture, coord + precalculated_1.xy).wx);
            float top_right = unpack_ufloat_16(tex2D(gm_BaseTexture, coord + precalculated_1.xw).wx);
            float bottom_left = unpack_ufloat_16(tex2D(gm_BaseTexture, coord + precalculated_1.zy).wx);
        ...
        OUT.color_0.wxyz = float4(pack_ufloat_16(color), 0.0, 0.0);
    } else {
